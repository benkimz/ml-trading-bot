...
from utils.colab_binance_api import *
...
class BotSession(BotTrader):
    def get_notional(self):
        if TESTNET == True:
            user = Client(api_key=TESTNET_API_KEY, api_secret=TESTNET_API_SECRET, testnet=True)
        else:
            user = Client(api_key=API_KEY, api_secret=API_SECRET, testnet=False)
                
        symbol_info = user.get_symbol_info(self.symbol)
    ...
    def __init__(self, base_asset=None, quote_asset=None):
        if base_asset == None or quote_asset == None:
            ...
            initial_balances = bal(base_asset, quote_asset)
            ...
            POSITION = False
            ##################
            notional_value = self.get_notional()
                ...
                while(True):
                    try:
                        ...
                        if POSITION == False and (prediction - reference) >= PROFIT_MARGIN:
                            buy(base_asset, quote_asset, 'auto')
                            base_balance, quote_balance = bal(base_asset, quote_asset)
                            ...
                            sell(base_asset, quote_asset, 'auto')
                            base_balance, quote_balance = bal(base_asset, quote_asset)
...
sss = BotSession(base_asset="BNB", quote_asset="USDT")
