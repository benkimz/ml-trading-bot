...
def bal(base=None, quote=None):
    if TESTNET == True:
        client = Client(api_key=TESTNET_API_KEY, api_secret=TESTNET_API_SECRET, testnet=True)
    else:
...
def buy(base=None, quote=None, max_amount='auto'):
    if TESTNET == True:
        client = Client(api_key=TESTNET_API_KEY, api_secret=TESTNET_API_SECRET, testnet=True)
    else:
        client = Client(api_key=API_KEY, api_secret=API_SECRET, testnet=False)
    ...
    symbol_info = client.get_symbol_info(pair_symbol)
    if symbol_info["isSpotTradingAllowed"] == True:
        ...
        notional_value = 0
        avg_price = client.get_avg_price(symbol=pair_symbol)
        ...
        if quote_balance < notional_value:
            ...
            if quantity > minQty and quantity < maxQty:
                try:
                    quantity = float(float_precision(quantity * 0.998, step_size))
...
def sell(base=None, quote=None, max_amount='auto'):
    if TESTNET == True:
        client = Client(api_key=TESTNET_API_KEY, api_secret=TESTNET_API_SECRET, testnet=True)
    else:
        client = Client(api_key=API_KEY, api_secret=API_SECRET, testnet=False)
